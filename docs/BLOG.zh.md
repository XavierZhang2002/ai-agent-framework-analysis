...
#### 模式一：标准同步循环（Standard Synchronous Loop）
...
#### 模式二：异步事件驱动（Asynchronous Event-Driven）
...
#### 模式三：三层 Actor 模型（Three-Layer Actor Model）
...
#### 模式四：增强型循环（Enhanced Loop with Capabilities）
...
**关键洞察**：实际实现中，步骤 4-6 可能有复杂的内部逻辑（并行工具、状态检查、安全检查等），但宏观上都是这个循环结构的变体。
...
┌─────────────┐          ┌─────────────┐          ┌─────────────┐
│   Host      │◄────────►│   Client    │◄────────►│   Server    │
│  (Agent)    │  stdio   │  (Transport)│  MCP协议  │  (Tools)    │
└─────────────┘          └─────────────┘          └─────────────┘
```
...
**关键洞察**：MCP 和直接函数调用并非对立关系，而是**解决不同问题**的方案。MCP 适合需要**广泛复用**的工具（如 GitHub、Slack 集成），而直接函数调用更适合**快速集成内部系统**。未来可能是两者并存的混合模式。
...
10. **AI Agent系列｜深入了解智能体工作流核心：Agent vs 传统编程 vs Workflow 的本质区别**（沈询，阿里云开发者，2025-02）  
