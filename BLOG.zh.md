...
#### 2.6 代码理解特性（Code Comprehension）

**特性：LSP 支持（Language Server Protocol）**

**代表性实现**（Claude Code、OpenCode）：
```typescript
// 通过 LSP 获取 IDE 级代码智能
...
// 获取定义、引用、重构等
const definition = await lspClient.gotoDefinition(file, line, column);
const references = await lspClient.findReferences(file, line, column);
```

**为什么重要**：LSP 提供 IDE 级别的代码理解能力：
- **跳转到定义**：理解符号关系
- **查找引用**：分析影响范围
- **重构支持**：安全的变量重命名、提取函数

**采用情况**：Claude Code、OpenCode、Crush、Cline CLI 已支持 LSP 集成。

**学术机会**：研究如何将 LSP 语义信息与 LLM 推理结合，提升代码修改的准确性。
...
class RepoMap:
    def get_ranked_tags(self, files, mentioned_tokens):
        """
        1. 使用 tree-sitter 解析代码库 AST
        2. 提取函数、类、变量定义
        3. 基于引用频率和修改时间排序
        4. 返回 Top 200-500 符号作为上下文
        """
        tags = self.parse_with_treesitter(files)
        return self.rank_by_relevance(tags, mentioned_tokens)
```

**为什么重要**：大型代码库无法全部放入 LLM 上下文（128K tokens 限制）。Repo Map 提供"代码库摘要"，让 LLM 理解项目结构而不需要看到每行代码。

**技术细节**：
- 支持 500+ 编程语言（tree-sitter 生态）
- 增量解析，只更新修改的文件
- 缓存到 `.aider.tags.cache.v3`

---

#### 2.7 项目上下文特性（Project Context）
...
**特性：Checkpoints / Plan Branching（检查点与计划分支）**

**Plandex 的代表性实现**：
```go
// 类 Git 的计划分支
plandex branch feature-refactor
plandex commit -m "重构用户认证模块"
plandex branch feature-alternative  // 创建替代方案分支
plandex checkout feature-refactor   // 切换回主分支
```

**Gemini CLI 的实现**：
- 自动在关键决策点创建检查点
- 用户可以"回滚"到之前的 Agent 状态
- 比较不同方案的差异

**为什么重要**：Agent 可能生成次优或错误的方案。检查点允许：
- **实验不同方案**：探索多个实现路径
- **安全回滚**：撤销不好的修改
- **方案对比**：可视化不同选择的差异
...
**观察**：
1. **各框架的特性组合呈现高度差异化**
2. **"Big Four" 第一梯队**（按 Stars）：
   - **OpenCode** (97.5K): 开源替代 Claude Code，75+ 提供商支持
   - **Gemini CLI** (93.6K): 最慷慨免费层（1000 请求/天）
   - **Claude Code** (64K): 功能最丰富，深度 IDE 集成
   - **Codex CLI** (59K): Rust 高性能，OS-level 沙箱
3. **每个框架都有相对独特的侧重特性**：
   - OpenAI SDK: HITL State, 完整 Tracing
   - Claude SDK: SDK MCP, 10+ Hooks, LSP
   - OpenCode: Context Compaction, Permission Ruleset, LSP
